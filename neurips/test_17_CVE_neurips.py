    ...
    with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for qi, m in enumerate(comparison_metrics):
            ...
            for tau_key in tau_keys:
                ...
                diff_str = "nan" if (isinstance(diff, float) and math.isnan(diff)) else f"{diff:.6f}"
                writer.writerow({
                    "query_id": qi + 1,
                    "query_text": m["query"],
                    "tau_method": tau_key,
...
def plot_semantic_recall_comparison(comparison_metrics, output_file=OUTPUT_DIR / "cve_semantic_recall_comparison.png"):
    ...
    for ti, (tau_key, tau_label) in enumerate(zip(tau_methods, tau_display)):
        ...
        ax2 = axes[ti, 2]
        uplift = [s - t for s, t, ok in zip(sem_vals, trad_vals, valid_mask) if ok]
        if uplift:
            ax2.hist(uplift, bins=min(15, len(uplift)), color=method_colors["semantic"], alpha=0.8, edgecolor="white")
