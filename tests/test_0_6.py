...
builder = (arrowspace.ArrowSpaceBuilder()
    .with_seed(42)
    .with_dims_reduction(enabled=True, eps=0.3)
    .with_sampling("simple", 0.8))
...
# EnergyMaps pipeline
energy_builder = (arrowspace.ArrowSpaceBuilder()
    .with_seed(999)
    .with_dims_reduction(enabled=True, eps=0.5)
    .with_sampling("simple", 0.8))

aspace_e, gl_e = energy_builder.build_energy(
    items,
    energy_params={"optical_tokens": 30, "w_lambda": 0.4, "w_dirichlet": 0.3},
    graph_params=graph_params
)
results_e = aspace_e.search_energy(query, gl_e, k=10)
